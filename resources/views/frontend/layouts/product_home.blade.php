        ...
        ($itemProductLatest->getProductVariant()->where('status', 0)->first()->price *
            ...
            <div id="best-sellar" role="tabpanel" class="tab-pane fade">
                <div id="popular-product" class="product-item-4 owl-carousel owl-theme nf-carousel-theme1">
                    <!-- item.1 -->
                    @foreach ($product_seller as $itemProductSeller)
                        <div class="product-item">
                            <div class="product-item-inner">
                                <div class="product-img-wrap">
                                    <img src="{{ $itemProductSeller->getProductVariant()->where('status', 0)->first()->image }}"
                                        alt="">
                                </div>
                                <div class="product-button">
                                    <a href="#" class="js_tooltip" data-mode="top" data-tip="Compare"><i
                            ...
                            <div class="product-detail">
                                <p class="product-title"><a
                                        href="{{ route('client.product.detail', $itemProductSeller->slug) }}">{{ $itemProductSeller->name }}</a>
                                </p>
                                <h5 class="item-price">
                                    <del>{{ number_format(
    $itemProductSeller->getProductVariant()->where('status', 0)->first()->price,
) }}
                                        Đ</del>
                                    {{ number_format(
    $itemProductSeller->getProductVariant()->where('status', 0)->first()->price -
        ($itemProductSeller->getProductVariant()->where('status', 0)->first()->price *
            $itemProductSeller->getProductVariant()->where('status', 0)->first()->discount) /
            100,
) }}
                                    Đ
                                </h5>
